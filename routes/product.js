        ...
        uploadProduct.fields([
            ...
            fields.forEach((field, index) => {
                if (req.files[field] && req.files[field].length > 0) {
                   ...
                   const serverIp = process.env.SERVER_IP || 'localhost'; // Par défaut "localhost" si non défini dans .env
                   const port = process.env.PORT || 3000; // Par défaut 3000 si non défini dans .env

              const imageUrl = file
            ? `http://${serverIp}:${port}/image/products/${file.filename}`
               : 'no_url';
