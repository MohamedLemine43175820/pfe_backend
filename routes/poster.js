    ...
    try {
        uploadPosters.single('img')(req, res, async function (err) {
            ...
            if (req.file) {
               ...
               const serverIp = process.env.SERVER_IP || 'localhost'; // Adresse IP depuis .env, par défaut "localhost"
               const port = process.env.PORT || 3000; // Port depuis .env, par défaut 3000
               imageUrl = `http://${serverIp}:${port}/image/poster/${req.file.filename}`;
            }
